...
import com.moviles.minkia.databinding.ActivityPermisosBinding
import com.moviles.minkia.ui.MainActivity
import kotlinx.coroutines.launch

/**
 * Pantalla de permisos (mockup C06). Es una pantalla de "priming": explica para
 * qué se usa cada acceso y recién entonces dispara los diálogos reales del
 * sistema. No bloquea: el usuario puede continuar aunque deniegue, porque cada
 * permiso se vuelve a pedir en contexto cuando haga falta (cámara al fotografiar,
 * ubicación al geolocalizar). Notificaciones es siempre opcional.
 */
class PermisosActivity : BaseActivity<ActivityPermisosBinding>() {
    ...
    override fun inflateBinding(inflater: LayoutInflater) = ActivityPermisosBinding.inflate(inflater)

    override fun onViewReady(savedInstanceState: Bundle?) {
        aplicarInsetsVerticales(binding.permisosRoot)
    ...
    private fun pintarChip(chip: TextView, concedido: Boolean, opcional: Boolean) {
        if (concedido) {
            ...
            chip.setTextColor(ContextCompat.getColor(this, R.color.verde_hoja))
            ...
            chip.setTextColor(ContextCompat.getColor(this, R.color.texto_secundario))
            ...
            chip.setTextColor(ContextCompat.getColor(this, R.color.naranja_terracota))
            chip.setCompoundDrawablesRelativeWithIntrinsicBounds(0, 0, 0, 0)
            chip.isClickable = true
        }
    }
    ...
    private fun irAInicio() {
        ...
        transicionFundido()
        finishAffinity() // cierra todo el flujo de acceso: no se vuelve con "atrás".
    }
}
